...
void moveEvenItemsToBack(LinkedList *ll)
{
	...
	ListNode *prev = NULL;
	ListNode *evenList = NULL, *evenListLast;
	prev = currNode;
	while (currNode != NULL) {	
		if (currNode->item%2==0) {
			prev->next = currNode->next;
			if (prev->next != NULL && prev->next->item%2!=0)	prev = prev->next;
			if (evenList == NULL) {
				evenList = currNode;
				evenListLast = evenList;
			} else {
				evenListLast->next = currNode;	
